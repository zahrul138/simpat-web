...
const MHLocalSchedulePage = ({ sidebarVisible }) => {
  ...
  const handleReturnVendorToIQC = async (vendorId) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/local-schedules/vendors/${vendorId}/return-to-iqc`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
        },
  ...
  const applyFilterLS_MHL = (arr, isSchedule = false) => {
    ...
    return arr.filter((item) => {
      ...
      if (by === "by_name")
        return getByFieldLS_MHL(item).toLowerCase().includes(kw);
      return true;
    });
  };
                                          ...
                                          <tr
                                            ...
                                            onClick={(e) => {
                                              if (
                                                ...
                                                toggleDetailHighlight(
                                                  `schedule_${schedule.id}_vendor_${vi}`,
                                                  part.id || pi,
                                                );
                                            }}
                                            style={{
                                              backgroundColor:
                                                highlightedDetailRows[
                                                  `schedule_${schedule.id}_vendor_${vi}_${part.id || pi}`
                                                ]
                                                  ? "#c7cde8"
                                                  : "transparent",
                                              cursor: "pointer",
